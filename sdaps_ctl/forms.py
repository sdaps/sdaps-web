...
class SurveyEditForm(SurveyModelForm):
    ...
    opts_checkmode = forms.ChoiceField(
        widget=forms.RadioSelect,
        label="Checkmode",
        choices=[('checkcorrect', 'checkcorrect'),
                 ('fill', 'fill'), ('check', 'check')],
    ...
    class Meta(SurveyModelForm.Meta):
        model = models.Survey
        fields = ('title', 'author', 'globalid', 'name', 'language', 'opts_noinfo',
                  'opts_paper_format', 'opts_print_questionnaire_id', 'opts_checkmode')
        ...
        self.helper.layout = Layout(
            Row(
                ...
                Row(
                    ...
                    Column('opts_print_questionnaire_id',
                           css_class='col-md-12'),
                    css_class='form-row col-md-5'
                ),
                Submit('submit', 'Update Survey',
                       css_class='btn btn-success form-row'),
    ...
    def save(self, commit=True):
        ...
        instance.set_tex_cls_opts(
            'paper_format', self.cleaned_data['opts_paper_format'])
        instance.set_tex_cls_opts(
            'print_questionnaire_id', self.cleaned_data['opts_print_questionnaire_id'])
        instance.set_tex_cls_opts(
            'checkmode', self.cleaned_data['opts_checkmode'])
        if commit:
            instance.save()
        return instance
