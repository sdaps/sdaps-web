...
class Survey(models.Model):
    """is the representation of an sdaps survey project within Django."""
    class Meta:
        permissions = (
            ("can_design_draft", "Can design a survey draft via the editor"),
            ("can_initialize",
             "Can finally setup/initialize the survey for printing"),
            ...
            ("can_download_project",
             "Can download the whole sdaps project for backup or working locally"),
            ("can_edit_basic_info",
             "Can edit for example title, author, global id..."),
        )
    ...
    @property
    def path(self):
        """returns the sdaps project path on filesystem of the survey
        instance."""
        survey_path = os.path.join(settings.SDAPS_PROJECT_ROOT, str(self.id))
        return survey_path

    def save(self, *args, **kwargs):
        """saves the django survey and creates a 6 characters long slug."""
        if not self.slug:
            self.slug = get_random_string(
                6, 'ABCDEFGHIJKLMNOPQVWXYZ0123456789')
        super().save(*args, **kwargs)

    def get_tex_cls_opts(self, key: str):
        """returns the value of sdaps latex class options, saved in
        latex_class_options via a given key (option name)."""
    ...
    def set_tex_cls_opts(self, key: str, value):
        """sets given sdaps latex class option via given key and value in
        json object in latex_class_options."""
        # change an option
...
class UploadedFile(models.Model):
    ...
    survey = models.ForeignKey(
        Survey, db_index=True, related_name="uploads", on_delete=models.CASCADE)

    def generate_filename(instance, filename: str):
        filename = get_valid_filename(instance.filename)
        new_filename = '%i-%s' % (instance.id, filename)
        return os.path.join(instance.survey.path, 'uploads', new_filename)

    file = models.FileField(max_length=255, editable=False,
                            upload_to=generate_filename, storage=settings.SDAPS_UPLOAD_STORAGE)
    ...
    def get_description(self):
        ...
        url = reverse('survey_upload_scans_file', args=(
            self.survey.slug, self.filename))
    ...
    def append_chunk(self, data, offset, length):
        ...
        scan_file = open(self.file.name, mode='ab')
...
def move_survey_dir(sender, instance, using, **kwargs):
    ...
    if path and os.path.isdir(path):
        ...
        os.rename(path, os.path.join(delpath, datetime.datetime.now().strftime(
            '%Y%m%d-%H%M') + '-' + str(instance.id)))


def assign_initial_survey_perms(sender, instance, created, **kwargs):
    survey_model_perms = [
        perm.codename for perm in get_perms_for_model(sender)]
